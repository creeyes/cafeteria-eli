...
export default function CoffeeMenu({ menuItems }: { menuItems: MenuData }) {
              ...
              <div className="grid gap-4">
                ...
                {menuItems.hotDrinks.simple.map((item, index) => (
                  <div key={index} className="flex items-start justify-between gap-2 sm:gap-4 py-2">
                    <div className="flex items-start gap-2 sm:gap-3">
                      ...
                      <div>
                        <h3 className="text-base sm:text-lg text-black dark:text-white">{item[`name_${language}`]}</h3>
                      </div>
                    </div>
                ...
                {menuItems.hotDrinks.withDescription.map((item, index) => (
                  <MenuItem
                    ...
                    description={item[`desc_${language}`] || undefined}
                    price={item.price}
                  />
                ))}
                ...
                {menuItems.craftBeers.map((beer, index) => {
                  return (
                    <MenuItem
                      key={index}
                      icon={<CupSoda size={18} />}
                      name={beer[`name_${language}`]}
                      description={beer[`desc_${language}`] || undefined}
                      price={beer.price}
                    />
                  )
                })}
